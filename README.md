# Horae - 时光记忆 v1.5.1 | SillyTavern 记忆增强插件
...
- **撤销操作**：删除事件、角色、物品、场景、关系等操作后可点击顶部撤销按钮恢复
- 系统提示词编辑器中的默认文本会根据当前启用的功能动态更新（如启用关系网络后 `<horae>` 标签格式区自动出现 `rel:` 行）

### UI / UX

- 修复 `--horae-surface` 未定义变量，改用 `--horae-bg-secondary`；情绪标签颜色改用 `var(--horae-accent)` 确保主题兼容。**已有的自定义美化不受影响**
- THEME.md 补充了新增选择器文档
