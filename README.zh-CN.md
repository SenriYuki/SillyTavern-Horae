# Horae - 时光记忆 v1.14.2 | SillyTavern 记忆增强插件
...
- **独立设置区块**：新增「辅助 API」，不依赖自动总结开关，单独填写 API 地址、密钥和模型。
- **三组用途开关**：可分别用于 AI 分析/魔术棒/发送前补全、自动总结/AI 智能补全、手动多选压缩。
- **串行队列**：辅助 API 请求会排队执行，降低多个后台任务同时触发 429 的概率。
- **默认不回退主 API**：辅助 API 失败时默认不偷跑主 API，避免继续撞主 API 速率限制；需要时可手动开启回退。
- **安全导出**：API 地址、密钥、模型不会进入 Horae 配置档导出内容，避免角色卡配置分享时泄露。
